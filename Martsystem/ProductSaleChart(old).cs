...
namespace MartSystem
{
    public partial class ProductSaleChart : Form
    {
        ...
        double totalQty, totalAmount;
        double maxQty=double.MaxValue, maxAmount=double.MaxValue;
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            dataReader = dataCon.ExecuteQry(sql);
            dataReader.Read();
            totalQty =Convert.ToInt32(dataReader.GetValue(0));
            totalAmount = Convert.ToInt32(dataReader.GetValue(1));
            dataCon.Con.Close();

            foreach (DataRow dr in dtResult.Rows)
            {
                double qty = dr.Field<double>("Quantity");
                double amount = dr.Field<double>("Amount");
                maxQty = Math.Max(maxQty, qty);
                maxAmount = Math.Max(maxAmount, amount);
            }
            maxQty = Math.Round(maxQty, 2);
        ...
        private void CbShowing_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            double max = cbShowing.Text == "Quantity" ? maxQty : maxAmount;
           

            double xMin = 1, xMax = 0, yMin = 0, yMax =max, xInterval = 1;
